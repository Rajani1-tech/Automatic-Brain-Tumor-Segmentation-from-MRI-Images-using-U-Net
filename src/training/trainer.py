...
class Trainer:
    def __init__(self, model, train_dir=Config.TRAIN_IMAGES_DIR,
                 train_mask=Config.TRAIN_MASKS_DIR,
                 val_dir=Config.VAL_IMAGES_DIR,
                 val_mask=Config.VAL_MASKS_DIR,
                 augment=True):
        self.model = model
        self.train_dataset = BrainTumorDataset(train_dir, train_mask, augment=augment)
        self.val_dataset = BrainTumorDataset(val_dir, val_mask, augment=False)
